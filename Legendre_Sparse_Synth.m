function [Y,P,A,V,D]=Legendre_Sparse_Synth(SNR,density,ModelType)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% [Y,P,A,V,D]=Legendre_Sparse_Synth(SNR,PSNR,ModelType)
%
% UUGS Spectral Library with linear or non-linear mixing models:
%
...
%prob=ones(Npixels,Npixels)*0.3;                          % (4) Probability of nonlinear mixing
prob=abs(randn(Npixels, Npixels)*0.3);
prob(prob>1)=1;
...
load('DatasetSynth/AbundancesLegendre64.mat')
a1=squeeze(A(:,:,1));
a2=squeeze(A(:,:,2));
a3=squeeze(A(:,:,3));
a4=squeeze(A(:,:,4));
...
if NoiseMeasurement==1 && NoiseSparse==1
     ...
     V(:,Iperp)=sprand(L,length(Iperp),density)*0.75*max(Yy(:))';    
end  
