...
xlabel('wavelength (nm)'); axis([min(wavelength) max(wavelength) min(P1(:)) max(P1(:))]);
ylabel('Normalized absorbance','FontSize',12)
title('(a) End-members','FontSize',12,'FontWeight','normal')

for i=1:4
    subplot(2,4,4+i);
    imagesc(reshape(A1(i,:),nRow,nCol),[0,1]);
...
subplot(2,4,8); colorbar('eastoutside')
annotation('textbox', [0.4, 0.5 0.3, 0.025], 'string', '(b) Abundances maps', 'FontSize', 12, 'FontWeight', 'normal', 'EdgeColor', 'none');
...
for i=1:4
    subplot(2,4,4+i);
    imagesc(reshape(A2(i,:),nRow,nCol),[0,1]);
    xlabel('pixels'); ylabel('pixels'); 
    if i>1 
        title(['Unknown-' num2str(i-1)],'FontWeight','normal');
    else
